...
onready var position_andre = get_node("Personagens/fase2_andre")
...
func _physics_process(delta):
	...
	#if step == 1 && positionRafa.position.x <= 512 && positionRafa.position.y >= 130 && position_andre == 1000:
		#position.x = 513
		#direction = Vector2(655 - position.x, 0)
		#if abs(direction.x) < speedAuto * delta:
			#direction.x = 655 # Parar no x = 490
			#step = 0
